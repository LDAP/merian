...
class ManagedVkImageOut : public VkImageOut, public AccessibleConnector<const ImageArrayResource&> {
  public:
    ManagedVkImageOut(const vk::AccessFlags2& access_flags,
  ...
  public:
    static ManagedVkImageOutHandle create(const vk::AccessFlags2& access_flags,
                                          const vk::PipelineStageFlags2& pipeline_stages,
                                          const vk::ImageLayout& required_layout,
                                          const vk::ShaderStageFlags& stage_flags,
                                          const vk::ArrayProxy<vk::ImageCreateInfo>& create_info,
    ...
    static ManagedVkImageOutHandle compute_write(const vk::Format format,
                                                 const vk::Extent3D extent,
                                                 const bool persistent = false);

    // uses the eGeneral Layout
    static ManagedVkImageOutHandle compute_fragment_write(const vk::Format format,
                                                          const vk::Extent3D extent,
                                                          const bool persistent = false);

    static ManagedVkImageOutHandle fragment_write(const vk::Format format,
                                                  const vk::Extent3D extent,
                                                  const bool persistent = false);

    static ManagedVkImageOutHandle color_attachment(const vk::Format format,
                                                    const vk::Extent3D extent,
                                                    const bool persistent = false);
    ...
    compute_read_write_transfer_dst(const vk::Format format,
                                    const vk::Extent3D extent,
    ...
    static ManagedVkImageOutHandle compute_read_write(const vk::Format format,
                                                      const vk::Extent3D extent,
                                                      const bool persistent = false);

    static ManagedVkImageOutHandle transfer_write(const vk::Format format,
                                                  const vk::Extent3D extent,
                                                  const bool persistent = false);
