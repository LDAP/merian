...
class ExtensionShadercCompiler : public ContextExtension, public GLSLCompilerProvider {
  public:
    static constexpr const char* name = "merian-shaderc-compiler";

    DeviceSupportInfo
    query_device_support([[maybe_unused]] const DeviceSupportQueryInfo& query_info) override {
        compiler = std::make_shared<ShadercCompiler>();
        if (!compiler->available())
            return DeviceSupportInfo{false, "shaderc not compiled in"};
