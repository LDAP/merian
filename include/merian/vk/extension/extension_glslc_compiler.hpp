...
class ExtensionGlslcCompiler : public ContextExtension, public GLSLCompilerProvider {
  public:
    static constexpr const char* name = "merian-glslc-compiler";

    DeviceSupportInfo
    query_device_support([[maybe_unused]] const DeviceSupportQueryInfo& query_info) override {
        compiler = std::make_shared<SystemGlslcCompiler>();
        if (!compiler->available())
            return DeviceSupportInfo{false, "glslc not found in PATH"};
