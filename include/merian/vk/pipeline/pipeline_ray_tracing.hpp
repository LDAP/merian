...
class RayTracingPipeline : public Pipeline {
  private:
    ...
    using ProcHitKey = std::tuple<VulkanEntryPointHandle,
    ...
    RayTracingPipeline(const PipelineLayoutHandle& pipeline_layout,
        ...
        : Pipeline(pipeline_layout->get_context(), pipeline_layout,
                   raygen_groups.empty()
                       ...
                       : flags),
          raygen_groups(std::move(raygen_groups)),
          miss_groups(std::move(miss_groups)),
          triangle_hit_groups(std::move(triangle_hit_groups)),
          procedural_hit_groups(std::move(procedural_hit_groups)),
          callable_groups(std::move(callable_groups)),
        ...
        for (const auto& g : this->triangle_hit_groups) {
            ...
            const uint32_t ahit =
                g.any_hit ? add_stage(*g.any_hit) : VK_SHADER_UNUSED_KHR;
            groups.push_back(vk::RayTracingShaderGroupCreateInfoKHR{
        ...
        for (const auto& g : this->procedural_hit_groups) {
            ...
            const uint32_t chit =
                g.closest_hit ? add_stage(*g.closest_hit) : VK_SHADER_UNUSED_KHR;
            const uint32_t ahit =
                g.any_hit ? add_stage(*g.any_hit) : VK_SHADER_UNUSED_KHR;
            groups.push_back(vk::RayTracingShaderGroupCreateInfoKHR{
                ...
                ahit,
                isect});
            proc_hit_tuple_map[{g.intersection,
                                 g.closest_hit.value_or(nullptr),
  ...
  public:
    ...
    create(const PipelineLayoutHandle&                        pipeline_layout,
        ...
        return RayTracingPipelineHandle(
            new RayTracingPipeline(pipeline_layout,
    ...
    uint32_t get_hit_count() const {
        return static_cast<uint32_t>(triangle_hit_groups.size() +
                                     procedural_hit_groups.size());
    }
