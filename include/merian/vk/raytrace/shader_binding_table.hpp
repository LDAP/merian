...
class ShaderBindingTableBuilder {
  public:
    ...
    ShaderBindingTableBuilder& add_miss(uint32_t group_index, const T& record_data) {
        return add_miss_raw(group_index, sizeof(T), reinterpret_cast<const char*>(&record_data));
    }
    ...
    ShaderBindingTableBuilder& add_hit(uint32_t group_index, const T& record_data) {
        return add_hit_raw(group_index, sizeof(T), reinterpret_cast<const char*>(&record_data));
    }
  ...
  private:
    ...
    add_miss_raw(uint32_t group_index, uint32_t data_size, const char* data);
    ShaderBindingTableBuilder&
    add_hit_raw(uint32_t group_index, uint32_t data_size, const char* data);
    ShaderBindingTableBuilder&
    add_callable_raw(uint32_t group_index, uint32_t data_size, const char* data);
