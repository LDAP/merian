...
class FileLoader {
  ...
  public:
    ...
    static BlobHandle load_file_as_blob(const std::filesystem::path& path) {
        return std::make_shared<StringBlob>(load_file_as_string(path));
    }

    static std::string load_file_as_string(const std::filesystem::path& path);
  ...
  public:
    FileLoader(const vk::ArrayProxy<std::filesystem::path>& search_paths = {"./"})
        : search_paths(search_paths.begin(), search_paths.end()) {}
    ...
    void add_search_path(const vk::ArrayProxy<std::filesystem::path>& path);

    void add_search_path(const vk::ArrayProxy<std::string>& path);
