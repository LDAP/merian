...
float vmf_pdf_wenzel(const vec3 w, const vec3 mu, const float kappa) {
    if (kappa < 1e-4) return INV_FOUR_PI;
    return kappa / (TWO_PI * (1.0 - exp(-2.0 * kappa))) * exp(kappa * (dot(w, mu) - 1.0));
}

vec3 vmf_sample_wenzel(const float kappa, const vec2 random) {
...
float vmf_pdf_tokuyoshi(const vec3 w, const vec3 mu, const float kappa) {
    const vec3 d = w - mu ;
    return exp (-0.5f * kappa * dot (d, d)) * x_over_expm1(-2.0f * kappa) / FOUR_PI;
}

vec3 vmf_sample_tokuyosh(const float kappa, const vec2 random) {
...
// Sample "around" z.
vec3 vmf_sample_tokuyosh(const vec3 z, const float kappa, const vec2 random) {
    return make_frame(z) * vmf_sample_tokuyosh(kappa, random);
}
