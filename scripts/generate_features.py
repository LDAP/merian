...
class FeatureStruct:
    ...
    promotion_version: Optional[str] = None  # e.g., VK_API_VERSION_1_2 (when extension became core)
...
def generate_header(features: list[FeatureStruct]) -> str:
    """Generate the feature.hpp header file content."""
    lines = [
...
def generate_implementation(features: list[FeatureStruct]) -> str:
    ...
    lines = [
        ...
        "// Do not edit manually!",
        "",
        '#include "merian/vk/extension/feature.hpp"',
        "",
    ...
    for feat in sorted(features, key=lambda f: f.cpp_name):
        ...
        lines.append("    std::vector<const char*> get_required_extensions(uint32_t vk_api_version) const override {")
        ...
        # get_feature_names
        lines.append("    std::vector<const char*> get_feature_names() const override {")
        lines.append("        return {")
        ...
        for member in feat.members:
            lines.append(f'        if (name == "{member.name}") return features.{member.name} == VK_TRUE;')
        lines.append("        return false;")
        lines.append("    }")
        lines.append("")

        # set_feature
        lines.append("    bool set_feature(const std::string& name, bool value) override {")
        for member in feat.members:
            lines.append(f'        if (name == "{member.name}") {{ features.{member.name} = value ? VK_TRUE : VK_FALSE; return true; }}')
        lines.append("        return false;")
        lines.append("    }")
        lines.append("")
        ...
        lines.append(f"    const vk::{feat.cpp_name}& get() const {{ return features; }}")
        lines.append("")
    ...
    lines.append("    static const std::unordered_map<std::string, std::function<FeatureHandle()>> feature_map = {")

    for feat in sorted(features, key=lambda f: f.cpp_name):
        class_name = generate_feature_class_name(feat.cpp_name)
        lines.append(f'        {{"{feat.cpp_name}", []() {{ return std::make_shared<{class_name}>(); }}}},')
        # Also add without the "Features" part for convenience
        short_name = feat.cpp_name
        for suffix in ["FeaturesKHR", "FeaturesEXT", "FeaturesNV", "FeaturesAMD",
                       "FeaturesARM", "FeaturesQCOM", "FeaturesINTEL", "FeaturesHUAWEI",
                       "FeaturesNVX", "FeaturesSEC", "FeaturesVALVE", "FeaturesMESA",
                       "FeaturesFUCHSIA", "FeaturesGOOGLE", "Features"]:
            if short_name.endswith(suffix):
                short_name = short_name.removesuffix(suffix)
                if short_name != feat.cpp_name:
                    lines.append(f'        {{"{short_name}", []() {{ return std::make_shared<{class_name}>(); }}}},')
                break
        # Add aliases (without PhysicalDevice prefix)
        for alias in feat.aliases:
            alias_cpp = vk_name_to_cpp_name(alias).removeprefix("PhysicalDevice")
            lines.append(f'        {{"{alias_cpp}", []() {{ return std::make_shared<{class_name}>(); }}}},')
...
def main():
    ...
    print(f"\nGenerating header file: {include_path / 'feature.hpp'}")
    header_content = generate_header(features)
    with open(include_path / "feature.hpp", "w") as f:
        f.write(header_content)

    print(f"Generating implementation file: {out_path / 'feature.cpp'}")
    impl_content = generate_implementation(features)
    with open(out_path / "feature.cpp", "w") as f:
