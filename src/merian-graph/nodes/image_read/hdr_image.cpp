...
HDRImageRead::describe_outputs([[maybe_unused]] const NodeIOLayout& io_layout) {
    ...
    try {
        ...
        image = image_load_f32(filename, image_info, 4);
    ...
    con_out = ManagedVkImageOut::create(vk::Format::eR32G32B32A32Sfloat, width, height, 1, true);
    needs_run = true;
    return {{"out", con_out, ConnectorAccess::transfer_dst}};
}

[[nodiscard]] HDRImageRead::NodeStatusFlags
HDRImageRead::process(const NodeIO& io, const NodeProcessInfo& info, Submission& submission) {
    if (!needs_run) {
        return {};
    }
    if (!image) {
