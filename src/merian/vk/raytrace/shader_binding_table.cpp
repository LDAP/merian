...
ShaderBindingTableHandle ShaderBindingTable::create(const RayTracingPipelineHandle& pipeline,
    ...
    for (vk::DeviceSize i = 0; i < n_hit; ++i) {
        ...
        const uint8_t* src = all_handles.data() + (abs_idx * handle_size);
        std::memcpy(data + hit_offset + (i * record_stride), src, static_cast<size_t>(handle_size));
    }
    ...
    if (n_raygen > 0)
        sbt->raygen_region = vk::StridedDeviceAddressRegionKHR{buf_addr + raygen_offset,
                                                               record_stride, record_stride};

    if (n_miss > 0)
        sbt->miss_region = vk::StridedDeviceAddressRegionKHR{buf_addr + miss_offset, record_stride,
                                                             miss_region_bytes};

    if (n_hit > 0)
        sbt->hit_region = vk::StridedDeviceAddressRegionKHR{buf_addr + hit_offset, record_stride,
                                                            hit_region_bytes};
...
ShaderBindingTableBuilder& ShaderBindingTableBuilder::add_miss_raw(uint32_t group_index,
                                                                   uint32_t data_size,
                                                                   const char* data) {
...
ShaderBindingTableBuilder&
ShaderBindingTableBuilder::add_hit_raw(uint32_t group_index, uint32_t data_size, const char* data) {
...
ShaderBindingTableBuilder& ShaderBindingTableBuilder::add_callable_raw(uint32_t group_index,
                                                                       uint32_t data_size,
                                                                       const char* data) {
...
ShaderBindingTableHandle ShaderBindingTableBuilder::build() {
    ...
    const vk::DeviceSize miss_stride =
        miss_records.empty() ? 0 : align_up(handle_size + miss_record_data_size, handle_alignment);

    const vk::DeviceSize hit_stride =
        hit_records.empty() ? 0 : align_up(handle_size + hit_record_data_size, handle_alignment);
    ...
    if (!raygen_records.empty())
        sbt->raygen_region = vk::StridedDeviceAddressRegionKHR{buf_addr + raygen_offset,
                                                               raygen_stride, raygen_stride};

    if (!miss_records.empty())
        sbt->miss_region = vk::StridedDeviceAddressRegionKHR{buf_addr + miss_offset, miss_stride,
                                                             miss_region_bytes};

    if (!hit_records.empty())
        sbt->hit_region =
            vk::StridedDeviceAddressRegionKHR{buf_addr + hit_offset, hit_stride, hit_region_bytes};
