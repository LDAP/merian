...
void AccelerationStructureBuilder::buildBLAS(const std::vector<BlasInput>& input,
    ...
    CommandPool cmd_pool(context, queue->get_queue_family_index());
    ...
    for (uint32_t i = 0; i < input_size; i++) {
        ...
        // Filling partially the vk::AccelerationStructureBuildGeometryInfoKHR for querying the
        // build sizes. Other information will be filled in the createBlas (see #2)
        build_info_i = vk::AccelerationStructureBuildGeometryInfoKHR{
        ...
        for (std::size_t tt = 0; tt < input[i].asBuildOffsetInfo.size(); tt++)
            maxPrimitiveCounts[tt] =
                input[i].asBuildOffsetInfo[tt].primitiveCount; // Number of primitives/triangles

        size_info_i = context->device.getAccelerationStructureBuildSizesKHR(
            vk::AccelerationStructureBuildTypeKHR::eDevice, build_info_i, maxPrimitiveCounts);

        // Extra info
        asTotalSize += size_info_i.accelerationStructureSize;
        maxScratchSize = std::max(maxScratchSize, size_info_i.buildScratchSize);
        nbCompactions +=
            (build_info_i.flags & vk::BuildAccelerationStructureFlagBitsKHR::eAllowCompaction) ? 1
                                                                                               : 0;
    }
    ...
    if (nbCompactions > 0) // Is compaction requested?
    {
        ...
        vk::QueryPoolCreateInfo qpci{
            {}, vk::QueryType::eAccelerationStructureCompactedSizeKHR, input_size};
        queryPool = context->device.createQueryPool(qpci);
    }

    // Batching creation/compaction of BLAS to allow staying in restricted amount of memory (when
    // compacting)
    ...
    for (uint32_t idx = 0; idx < input_size; idx++) {
        ...
        if (batch_device_size >= batchLimit || idx == input_size - 1) {
            ...
            // BUILD BATCH
            vk::CommandBuffer cmdBuf = cmd_pool.create_and_begin();
    ...
    if (queryPool.has_value()) {
        vk::DeviceSize compactSize = std::accumulate(
            build_infos.begin(), build_infos.end(), 0ULL,
            [](const auto& a, const auto& b) { return a + b.sizeInfo.accelerationStructureSize; });
        const float fractionSmaller =
            (asTotalSize == 0) ? 0 : (asTotalSize - compactSize) / float(asTotalSize);
...
void AccelerationStructureBuilder::cmdCreateBLAS(
    ...
    if (queryPool.has_value()) // For querying the compaction size
        context->device.resetQueryPool(queryPool.value(), 0, static_cast<uint32_t>(indices.size()));
    uint32_t queryCnt = 0;
    ...
    for (const auto& idx : indices) {
        ...
        if (queryPool.has_value()) {
            ...
            cmdBuf.writeAccelerationStructuresPropertiesKHR(
                ...
                vk::QueryType::eAccelerationStructureCompactedSizeKHR, queryPool.value(),
                queryCnt++);
        }
    }
}

void AccelerationStructureBuilder::cmdCompactBLAS(
    vk::CommandBuffer& cmdBuf,
    std::vector<uint32_t>& indices,
    std::vector<BuildAccelerationStructureInfo>& buildAs,
    ...
    check_result(context->device.getQueryPoolResults(queryPool, 0, (uint32_t)compactSizes.size(),
...
void AccelerationStructureBuilder::updateBLAS(uint32_t blasIdx,
    ...
    for (std::size_t tt = 0; tt < blas.asBuildOffsetInfo.size(); tt++)
        maxPrimCount[tt] =
            blas.asBuildOffsetInfo[tt].primitiveCount; // Number of primitives/triangles
    ...
    vk::CommandBuffer cmdBuf = genCmdBuf.create_and_begin();
...
void AccelerationStructureBuilder::buildTLAS(
...
void AccelerationStructureBuilder::cmdCreateTLAS(vk::CommandBuffer& cmdBuf,
    ...
    // Put the above into a VkAccelerationStructureGeometryKHR. We need to put the instances struct
    // in a union and label it as instance data.
    ...
    vk::BuildAccelerationStructureModeKHR mode =
        update ? vk::BuildAccelerationStructureModeKHR::eBuild
               : vk::BuildAccelerationStructureModeKHR::eBuild;
