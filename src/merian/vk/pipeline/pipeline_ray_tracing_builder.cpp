...
RayTracingPipelineBuilder& RayTracingPipelineBuilder::set_max_recursion_depth(uint32_t depth) {
...
RayTracingPipelineHandle RayTracingPipelineBuilder::build(const PipelineLayoutHandle& layout,
    ...
    return RayTracingPipeline::create(
        layout, std::move(raygen_groups), std::move(miss_groups), std::move(triangle_hit_groups),
        std::move(procedural_hit_groups), std::move(callable_groups), libraries,
        max_recursion_depth, std::move(dynamic_states), flags, base_pipeline, pNext);
}
