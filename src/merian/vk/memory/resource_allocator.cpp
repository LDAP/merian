...
BufferHandle ResourceAllocator::createBuffer(const vk::CommandBuffer& cmdBuf,
                                             ...
                                             const std::string& debug_name,
                                             const std::optional<vk::DeviceSize> min_alignment) {
    BufferHandle resultBuffer = createBuffer(size_, usage_, mapping_type, debug_name, min_alignment);
...
BufferHandle ResourceAllocator::createScratchBuffer(const vk::DeviceSize size,
                                                    ...
                                                    const std::string& debug_name) {
    return createBuffer(size, vk::BufferUsageFlagBits::eShaderDeviceAddress | vk::BufferUsageFlagBits::eStorageBuffer,
...
ImageHandle ResourceAllocator::createImage(const vk::CommandBuffer& cmdBuf,
    ...
    if (data_ != nullptr) {
        ...
        m_staging->cmdToImage(cmdBuf, *resultImage, offset, info_.extent, subresource, size_, data_);

        // Setting final image layout
        merian::cmd_barrier_image_layout(cmdBuf, *resultImage, vk::ImageLayout::eTransferDstOptimal, layout_);
        resultImage->_set_current_layout(layout_);
    } else {
        // Setting final image layout
        merian::cmd_barrier_image_layout(cmdBuf, *resultImage, vk::ImageLayout::eUndefined, layout_);
    }
...
ResourceAllocator::createAccelerationStructure(const vk::AccelerationStructureTypeKHR type,
    ...
    // Setting the buffer
    vk::AccelerationStructureCreateInfoKHR createInfo{{}, *buffer, {}, size_info.accelerationStructureSize, type};
