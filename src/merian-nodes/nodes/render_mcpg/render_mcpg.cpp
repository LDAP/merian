#include "merian-nodes/nodes/render_mcpg/render_mcpg.hpp"
...
RenderMCPGNode::RenderMCPGNode() = default;

DeviceSupportInfo RenderMCPGNode::query_device_support(const DeviceSupportQueryInfo& query_info) {
    return DeviceSupportInfo::check(query_info, {}, {"rayQuery"});
}

void RenderMCPGNode::initialize(const ContextHandle& context,
...
std::vector<InputConnectorDescriptor> RenderMCPGNode::describe_inputs() {
    return {{"scene", con_scene}, {"gbuffer", con_gbuffer}};
}

std::vector<OutputConnectorDescriptor>
RenderMCPGNode::describe_outputs([[maybe_unused]] const NodeIOLayout& io_layout) {
    con_irradiance = ManagedVkImageOut::compute_write(vk::Format::eR32G32B32A32Sfloat, extent);
    return {{"irradiance", con_irradiance}};
}

RenderMCPGNode::NodeStatusFlags RenderMCPGNode::on_connected(
...
void RenderMCPGNode::process(GraphRun& run,
    ...
    if (!program) {
        ...
        composition->add_module_from_path("merian-nodes/nodes/render_mcpg/render_mcpg.slang", true);
...
RenderMCPGNode::NodeStatusFlags RenderMCPGNode::properties(Properties& config) {
